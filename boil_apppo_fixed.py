...
class BOiLApp(ctk.CTk):
    ...
    def _build_sidebar(self):
        ...
        ctk.CTkLabel(self.sidebar,
                     text=("Kroki algorytmu:\n"
                           ...
                           "② Bilansowanie: FD lub FO\n"
                           "③ Plan bazowy: wierzchołek NW\n"
                           "④ Zmienne dualne α_i, β_j\n"
    ...
    def _run(self):
        try:
            ...
            # 3. Bilansowanie — FD jeśli sum_d > sum_s, FO jeśli sum_s > sum_d
            sum_s = sum(supply_orig)
            sum_d = sum(demand_orig)

            if sum_d > sum_s:
                # Dodaj fikcyjnego dostawcę (FD) z podażą = niedobór
                fd_supply = sum_d - sum_s
                supply = list(supply_orig) + [fd_supply]
                demand = list(demand_orig)
                row_names = [f"D{i+1}" for i in range(S0)] + ["FD"]
                col_names = [f"O{j+1}" for j in range(C0)]
                # Z matrix: dodaj wiersz FD z zerami
                z_pen = np.vstack([z_orig, np.zeros((1, C0))])
                z_orig_full = z_pen.copy()
                fd_row_idx = S0
                if blocked_col is not None:
                    z_pen[fd_row_idx, blocked_col] = -M_BIG
            elif sum_s > sum_d:
                # Dodaj fikcyjnego odbiorcę (FO) z popytem = nadwyżka
                fo_demand = sum_s - sum_d
                supply = list(supply_orig)
                demand = list(demand_orig) + [fo_demand]
                row_names = [f"D{i+1}" for i in range(S0)]
                col_names = [f"O{j+1}" for j in range(C0)] + ["FO"]
                # Z matrix: dodaj kolumnę FO z zerami
                z_pen = np.hstack([z_orig, np.zeros((S0, 1))])
                z_orig_full = z_pen.copy()
                fd_row_idx = None  # brak FD
            else:
                # Zbilansowane — bez fikcyjnych
                supply = list(supply_orig)
                demand = list(demand_orig)
                row_names = [f"D{i+1}" for i in range(S0)]
                col_names = [f"O{j+1}" for j in range(C0)]
            ...
            # 4. Plan bazowy: metoda wierzchołka północno-zachodniego (NW)
            alloc0 = nw_corner(supply, demand)

            # Jeśli zablokowana komórka dostała alokację, wyzeruj ją przed MODI
            if blocked_col is not None and fd_row_idx is not None:
            ...
            # 6. Wyświetlenie
            self._clear_results()
            self._show_z_matrix(z_orig_full, z_pen, row_names, col_names, blocked_col if fd_row_idx is not None else None)
            self._show_iterations(iterations, z_orig_full, z_pen, row_names, col_names, blocked_col if fd_row_idx is not None else None)
            self._show_summary(iterations, z_orig_full, row_names, col_names,
                               sell, trans, buy, blocked_col if fd_row_idx is not None else None)
    ...
    def _show_summary(self, iterations, z_orig, row_names, col_names,
        ...
        n_real_cols = sum(1 for c in col_names if c.startswith("O"))
